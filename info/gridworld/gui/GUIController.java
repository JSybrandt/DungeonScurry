...
public class GUIController<T>
{
        ...
        display.addMouseListener(new MouseAdapter()
            {
                public void mousePressed(MouseEvent evt)
                {
                    ...
                    Actor a = (Actor) parentFrame.getWorld().getGrid().get(loc);
                    if(GUI.getPlayer().checkLife())
                    {

                        if (loc != null && a instanceof Monster)
                        {
                            GUI.getPlayer().fight((Being)parentFrame.getWorld().getGrid().get(loc));
                            parentFrame.repaint();
                        }

                        if (loc != null && a instanceof Weapon && GUI.getPlayer().checkRange(a)==1)
                        {
                            GUI.getPlayer().equip((Weapon)a);
                            a.removeSelfFromGrid();
                            parentFrame.repaint();
                        }
                        if (loc != null && a instanceof Armor && GUI.getPlayer().checkRange(a)==1)
                        {
                            GUI.getPlayer().equip((Armor)a);
                            a.removeSelfFromGrid();
                            parentFrame.repaint();
                        }
                        if (loc != null && a instanceof Consumable && GUI.getPlayer().checkRange(a)==1)
                        {
                            Consumable b = (Consumable) a;
                            b.consume();
                            parentFrame.repaint();
                        }
    ...
    private void locationClicked()
    {
        ...
        if (loc != null && !world.locationClicked(loc) && world.getGrid().get(loc) instanceof Monster)
            GUI.getPlayer().fight((Being)world.getGrid().get(loc));
        parentFrame.repaint();
    }
    ...
    {
    GUI.getPlayer().fight((Being)occupant);
    }
    }
